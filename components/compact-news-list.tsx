  ...
  return (
    <section className="rounded-3xl border border-border/70 bg-card p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between border-b border-border/70 pb-4">
        <Link href={href} className="text-2xl font-semibold text-primary">{title}</Link>
        <SlidersHorizontal className="h-5 w-5 text-muted-foreground" />
      </div>
      <div className="divide-y divide-border/70">
        {items.map((item) => (
          <Link key={item.id} href={`/brief/${item.id}`} className="grid gap-3 py-4 first:pt-0 sm:grid-cols-[1fr_92px]">
            <div>
              <SourceBadge brief={item} />
              <h3 className="mt-2 text-lg font-semibold leading-snug hover:text-primary">{item.title}</h3>
